...
async function handleMapClick(lat, lng) { 
    try { 
        ...
        const name = data.success ? data.address : `Coordenadas: ${lat.toFixed(6)}, 
${lng.toFixed(6)}`; 
...
function showNotification(message, type = 'info') { 
    ...
    notification.className = `notification fixed top-4 right-4 ${colors[type]} text-white px-6 py-3 
rounded-lg shadow-lg z-50 transform transition-transform duration-300 translate-x-0`; 
